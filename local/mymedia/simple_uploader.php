...
$PAGE->set_url('/local/mymedia/simple_uploader.php');
...
if (!$connection) {  // When connection failed.
    ...
    $uploadurl = local_kaltura_get_host() . '/api_v3/service/uploadToken/action/upload';
    ...
    $output = '';
    $output .= '<script type="text/javascript" src="/moodle/local/mymedia/js/simple_uploader_2pass.js"></script>';

    if ($ks == null || empty($rootpath)) { // Session failed.
        $output .= get_string('session_failed', 'local_mymedia');
        ...
        $output .= get_string('default_access_control_failed', 'local_mymedia');
        $output .= html_writer::empty_tag('br');
        $attr = array('type' => 'button', 'name' => 'uploader_cancel', 'id' => 'uploader_cancel',
                      'value' => 'Back', 'onclick' => 'handleCancelClick();');
        $output .= html_writer::empty_tag('input', $attr);
        ...
        $output .= '<script type="text/javascript" src="/moodle/local/kaltura/js/encoding.js"></script>';
