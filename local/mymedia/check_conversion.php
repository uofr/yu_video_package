...
if (0 == strcmp($widget, 'kdp')) {
    ...
    $entryobj = local_kaltura_get_ready_entry_object($entryid, false);

    if (empty($entryobj)) { // Sometimes the connection to Kaltura times out.
        $data->markup = get_string('media_retrival_error', 'local_mymedia');
        die;
    }

    // Determine the type of media (See KALDEV-28).
    if (!local_kaltura_media_type_valid($entryobj)) {
        $entryobj = local_kaltura_get_ready_entry_object($entryobj->id, false);
    }
    ...
    if (KalturaEntryStatus::READY == (string) $entryobj->status) {
        ...
        $session  = local_kaltura_generate_kaltura_session(array($entryobj->id));

        $data->markup = local_kaltura_get_kdp_code($entryobj, $uiconfid, $courseid, $session);
        ...
        switch ((string) $entryobj->status) {
            case KalturaEntryStatus::ERROR_IMPORTING:
                $data->markup = get_string('media_error', 'local_mymedia');
                break;
            case KalturaEntryStatus::ERROR_CONVERTING:
                $data->markup = get_string('media_error', 'local_mymedia');
                break;
            case KalturaEntryStatus::INFECTED:
                $data->markup = get_string('media_bad', 'local_mymedia');
                break;
