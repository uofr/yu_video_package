...
function local_kaltura_initialize_account($login, $password, $uri = '') {
    ...
    try {
        ...
        $configobj->clientTag = local_kaltura_create_client_tag();
        ...
        $configobj->serviceUrl = local_kaltura_get_host();
        $configobj->cdnUrl = local_kaltura_get_host();
        ...
        if (0 == strcmp('hosted', $connectiontype)) {
            ...
            local_kaltura_send_initialization($adminsession);
 ...
 */
function local_kaltura_login($admin = false, $privileges = '', $expiry = 10800, $testconn = false) {
    global $USER;
    ...
    $configobj = local_kaltura_get_configuration_obj();
    ...
    if (!empty($session)) {
        ...
        if ($testconn) {
            $result = local_kaltura_test_connection($clientobj);
 ...
 */
function local_kaltura_generate_weak_kaltura_session($courseid, $coursename) {
    global $CFG, $USER, $DB;

    $configobj = local_kaltura_get_report_configuration_obj();
    ...
    $kalcategory = local_kaltura_create_user_category($connection, $USER->username);
 ...
 */
function local_kaltura_generate_kaltura_session($medialist = array()) {
    global $USER;
 ...
 */
function local_kaltura_get_kdp_code($entryobj, $uiconfid = 0, $courseid = 0, $session = '', $uid = 0) {
 ...
 *
 */
function local_kaltura_get_kdp_flashvars($creatorname = '', $session = '') {
    global $USER;
    ...
    $category = local_kaltura_create_user_category($connection, $creatorname);
 ...
 */
function local_kaltura_is_valid_entry_object($entryobj) {
 ...
 */
function local_kaltura_get_ready_entry_object($entryid, $readyonly = true) {
...
function local_kaltura_test_connection($clientobj) {
    $results = false;
 ...
 */
function local_kaltura_htm5_javascript_url($uiconfid) {
 ...
 */
function local_kaltura_update_media_metadata($connection, $entryid, $param) {
 ...
 */
function local_kaltura_delete_media($connection, $entryid) {
