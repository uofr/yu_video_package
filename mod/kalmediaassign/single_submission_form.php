    ...
    public function definition() {
        ...
        if (!empty($submission->entry_id)) {
            ...
            if ($connection) {
                ...
                if (!local_kaltura_media_type_valid($entryobject)) {
        ...
        if (!empty($entryobject)) {
            ...
            if (KalturaMediaType::IMAGE == $entryobject->mediaType) {
                $markup = local_kaltura_create_image_markup($entryobject, $entryobject->name, $session, $theme);
            } else {
                list($entryobject->width, $entryobject->height) = kalmediaassign_get_player_dimensions();
                if (0 == strcmp($theme, 'mymobile')) {
                        $markup = local_kaltura_get_kwidget_code($entryobject, $uiconfid, $courseid, $session);
                } else {
                    $markup = local_kaltura_get_kdp_code($entryobject, $uiconfid, $courseid, $session);
                }
            }
            ...
            if ($connection) {
                ...
                $mform->addElement('static', 'description', get_string('submission', 'kalmediaassign'),
                                   get_string('media_converting', 'local_kaltura'));
            } else {
