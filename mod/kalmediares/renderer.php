...
class mod_kalmediares_renderer extends plugin_renderer_base {
    ...
    public function embed_media($kalmediares) {
        ...
        $output = '';
        $entryobj = local_kaltura_get_ready_entry_object($kalmediares->entry_id);

        if (!empty($entryobj)) {

            // Check if player selection is globally overridden.
            if (local_kaltura_get_player_override()) {
                $newplayer = local_kaltura_get_player_uiconf('player_resource');
            ...
            if (KalturaMediaType::IMAGE == $entryobj->mediaType) {
                $markup = local_kaltura_create_image_markup($entryobj, $kalmediares->name, $theme,
                ...
                if (0 == strcmp($theme, 'mymobile')) {
                    $markup = local_kaltura_get_kwidget_code($entryobj, $kalmediares->uiconf_id, $courseid, $session);
                } else {
                    $markup = local_kaltura_get_kdp_code($entryobj, $kalmediares->uiconf_id, $courseid, $session);
                }
            }
